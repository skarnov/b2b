...
<div class="right_col" role="main">
        ...
        <div class="title_right">
            <div class="form-group pull-right top_search">
                <div class="btn-group btn-group-sm" role="group">
                    <a href="<?= base_url() ?>/domain" class="btn btn-secondary btn-xs">Domain Management</a>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="row">
        <div class="col-md-12 col-sm-12">
            <form id="domain" method="post" data-parsley-validate class="form-horizontal form-label-left">
                <div class="x_panel">
                    <div class="x_content">
                        ...
                        <div class="col-md-2">
                            <div class="nav nav-tabs flex-column bar_tabs" id="v-pills-tab" role="tablist" aria-orientation="vertical">
                                ...
                                <a class="nav-link" id="v-pills-panel-tab" data-toggle="pill" href="#v-pills-panel" role="tab" aria-controls="v-pills-panel" aria-selected="false">Domain Panel</a>
                        ...
                        <div class="col-md-10">
                            <div class="tab-content" id="v-pills-tabContent">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <select name="partner_id" required class="form-control">
                                                ...
                                                <?php foreach ($all_partners as $value) : ?>
                                                    <option value="<?= $value['partner_id'] ?>" <?= $domain_info['fk_partner_id'] == $value['partner_id'] ? 'selected' : '' ?>><?= $value['partner_name'] ?></option>
                                                <?php endforeach ?>
                                            </select>
                                        </div>
                                        <div class="form-group">
                                            <label>Domain Name <span class="required">*</span></label>
                                            <input type="hidden" name="domain_id" value="<?= $domain_info['domain_id'] ?>">
                                            <input type="text" readonly required="required" value="<?= $domain_info['domain_name'] ?>" class="form-control">
                                        </div>
                                        <div class="form-group">
                                            <label>Domain Email <span class="required">*</span></label>
                                            <input type="text" name="domain_email" required="required" value="<?= $domain_info['domain_email'] ?>" class="form-control">
                                        </div>
                                        <div class="form-group">
                                            <label>Renew Date <span class="required">*</span></label>
                                            <input type="date" name="renew_date" required="required" value="<?= $domain_info['renew_date'] ?>" class="form-control">
                                        </div>
                                    </div>
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            ...
                                            <select name="user_id" required class="form-control">
                                                ...
                                                <?php foreach ($all_users as $value) : ?>
                                                    <option value="<?= $value['user_id'] ?>" <?= $domain_info['fk_user_id'] == $value['user_id'] ? 'selected' : '' ?>><?= $value['first_name'] . ' ' . $value['last_name'] ?></option>
                                                <?php endforeach ?>
                                            </select>
                                        </div>
                                        <div class="form-group">
                                            <label>Domain Mobile <span class="required">*</span></label>
                                            <input type="number" name="domain_mobile" required="required" value="<?= $domain_info['domain_mobile'] ?>" class="form-control">
                                        </div>
                                        <div class="form-group">
                                            <label>Status</label>
                                            <select name="status" class="form-control">
                                                <option value="active" <?= $domain_info['domain_status'] == 'active' ? 'selected' : '' ?>>Active</option>
                                                <option value="inactive" <?= $domain_info['domain_status'] == 'inactive' ? 'selected' : '' ?>>Inactive</option>
                                                <option value="deleted" <?= $domain_info['domain_status'] == 'deleted' ? 'selected' : '' ?>>Deleted</option>
                                            </select>
                                        </div>
                                        <div class="form-group">
                                            <label>Expiry Date <span class="required">*</span></label>
                                            <input type="date" name="expiry_date" required="required" value="<?= $domain_info['expiry_date'] ?>" class="form-control">
                                        </div>
                                    </div>
                                </div>
                                <div class="tab-pane fade" id="v-pills-panel" role="tabpanel" aria-labelledby="v-pills-panel-tab">
                                    <div class="col-md-8">
                                        <div class="form-group">
                                            <label>Domain Panel Access URL <span class="required">*</span></label>
                                            <input type="text" name="panel_url" required="required" value="<?= $domain_info['panel_url'] ?>" class="form-control">
                                        </div>
                                        <div class="form-group">
                                            <label>Domain Panel Username <span class="required">*</span></label>
                                            <input type="text" name="panel_username" required="required" value="<?= $domain_info['panel_username'] ?>" class="form-control">
                                        </div>
                                        <div class="form-group">
                                            <label>Domain Panel Password <span class="required">*</span></label>
                                            <input type="text" name="panel_password" required="required" value="<?= $domain_info['panel_password'] ?>" class="form-control">
                                        </div>
                                    </div>
                                </div>
                                <div class="tab-pane fade" id="v-pills-comment" role="tabpanel" aria-labelledby="v-pills-comment-tab">
                                    <div class="col-md-12">
                                        <div class="form-group">
                                            <label>Note</label>
                                            <textarea name="note" id="note" class="form-control"><?= $domain_info['domain_note'] ?></textarea>
                                        </div>
                                    </div>
                                </div>
                    ...
                    <div class="col-md-12">
                        ...
                        <div class="form-group">
                            <button type="submit" id="submit" class="btn btn-sm btn-primary">Renew - <?= $domain_info['domain_name'] ?></button>
                        </div>
                    </div>
                </div>
            </form>
...
<script>
    jQ.push(function () {
        ...
        $('#submit').click(function (e) {
            ...
            $.ajax({
                type: 'POST',
                url: '<?= base_url() ?>/domain/domain_renew',
                ...
                success: function (data)
                {
                    ...
                    $("#submit").html('Renew - <?= $domain_info['domain_name'] ?>');
                }
            });
        });
    });
